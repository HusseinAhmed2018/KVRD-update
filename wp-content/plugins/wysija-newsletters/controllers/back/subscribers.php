    ...
    function export_get(){
        ...
        // $objReader->setDelimiter("\t");
         ...
         function detectDelimiter($csvFile)
          ...
          $del = detectDelimiter($file_path_result["path"]);

          $objReader->setDelimiter($del);
        ...
        // die();
         ...
         $objPHPExcel = $objReader->load($file_path_result["path"]);

         $trimmed_url = trim($file_path_result['url'], ".csv").'.xls';
         $trimmed_path = trim($file_path_result['path'], ".csv").'.xls';
         $file_path_result['url'] = $trimmed_url;
         $file_path_result['path'] = $trimmed_path;
         
         $objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel5');
         $objWriter->save($trimmed_path);
        ...
        if(isset($_REQUEST['camp_id'])){
            $filecsv = $this->redirect('admin.php?page=wysija_campaigns&action=viewstats&id='.$_REQUEST['camp_id']);
